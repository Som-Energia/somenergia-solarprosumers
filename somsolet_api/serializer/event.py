...
class RenkontoEventSerializer(serializers.ModelSerializer):
    ...
    date_start = serializers.DateTimeField(source="start", format="%Y-%m-%dT%H:%M:%S%z")

    date_end = serializers.DateTimeField(source="end", format="%Y-%m-%dT%H:%M:%S%z")
    ...
    def set_technical_visit(self, calendar, project):
        ...
        technical_visit = self.create(event_data={**event_data, **self.validated_data})
