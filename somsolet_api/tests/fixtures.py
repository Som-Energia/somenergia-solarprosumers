...
from somrenkonto.factories import (
    CalendarFactory,
    CampaignFactory,
    ProjectFactory,
...
def engineering_with_events():
    ...
    bounded_event_data = dict(
        title=factory.Iterator(
            ["Super urgent meet", "Meeting with Escayola", "Vacations"]
        ),
    ...
    RenkontoEventFactory.create_batch(size=5, **bounded_event_data)
...
def stats():
    stats_data = dict(
        ...
        total_power=dict(value=30, units="kWp"),
        total_generation=dict(value=0.0435, units="GWp/any"),
    )
    return stats_data
