...
class TestMailing:
    ...
    def test_send_pending_notification__base_case(self, mailing_signature):
        scheduler_tasks.send_pending_notification()
        email_template = "emails/signature.html"

        assert len(mail.outbox) == 1
        email = mail.outbox[0]
        assert email.subject == _(
            f"CONTRACTE CLAU EN MÀ [Instalació plaques Montserrat Escayola] - Solar Paco, compra col·lectiva de Som Energia"
        )
    ...
    def test_send_pending_notification__with_two_attachments(self, mailing_offer):
        scheduler_tasks.send_pending_notification()
