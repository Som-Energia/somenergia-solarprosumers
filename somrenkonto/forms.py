...
class CalendarForm(forms.Form):
    name = forms.CharField(
        max_length=256, required=True, help_text=_("Name of the calendar")
    )


class RenkontoEventForm(forms.Form):
    title = forms.CharField(
        max_length=256, required=True, help_text=_("Tittle of the event")
    )

    description = forms.CharField(
        required=False, widget=forms.Textarea, help_text=_("Description of the event")
    )
    ...
    event_type = forms.ChoiceField(
        choices=EventChoices.choices, required=True, help_text=_("Type of the event")
    )

    calendar = forms.IntegerField(
        required=True, help_text=_("Calendar ID for this event")
    )
...
class CalendarConfigForm(forms.ModelForm):
    working_days = forms.ChoiceField(
        choices=WeekDays.choices, help_text=_("Type of the event")
    )
