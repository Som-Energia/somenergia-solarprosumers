...
class TestFilterViewMixin:
    ...
    def test__get_filter_params__multiple_value(self, rf):
        ...
        assert filter_params == [Q(campaign_id__in=[1, 2]), Q(project_id__in=[5, 6])]
    ...
    def test__get_filter_params__multiple_value_srting_type(self, rf):
        ...
        assert filter_params == [
            Q(campaign_id__in=[1, 2]),
            Q(event_type__in=["APPO", "UNAVAIL"]),
        ]
...
class TestCalendarView:
    ...
    def test__edit_calendar_conf__button_in_calendar_view(
        ...
        conf_calendar_url = reverse("edit_calendar", kwargs={"pk": calendar.id})
...
class TestRenkontoEventQuerySet:
    ...
    def test__engineering_events(self, authenticated_user, engineering_with_events):
        ...
        assert list(events) == list(
            RenkontoEvent.objects.filter(engineering__id=engineering_id)
        )
