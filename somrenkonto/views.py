...
class FilterViewMixin(object):
    def get_filter_params(self, request):
        ...
        params = {
            ...
            for field, type_ in self.FILTER_FIELDS
...
class CalendarView(FilterViewMixin, View):
    FILTER_FIELDS = [("campaign_id", int), ("project_id", int), ("event_type", str)]
    ...
    def _get_campaigns(self, request):
        ...
        return {
            campaign.name: {
                "id": campaign.id,
                "installations": list(
                    Project.objects.filter(campaign=campaign).values("name", "id")
                ),
            }
    ...
    def _get_calendar_events(self, request):
        ...
        filters.append(Q(campaign__in=Campaign.campaigns.user_campaigns(request.user)))
        return RenkontoEvent.events.filter_events(filters)

    def _create_calendar(self, name):
        calendar = Calendar(name=name, slug=slugify(name))
        calendar.save()
        CalendarConfig.objects.get_or_create(calendar=calendar)
        return calendar
    ...
    def get(self, request):
        try:
            ...
            return self._show_calendar_view(
                request, calendar=ing_calendar, template="calendar.html"
            )
