...
class SomsoletProjectView(LoginRequiredMixin, View):
    ...
    def get_initial(self, pk, engineering):
        proj_inst = get_object_or_404(
            Project, pk=pk, campaign__engineerings=engineering
        )
        return {
    ...
    def get(self, request, pk):
        ...
        if self.initial["status"] in self.status_condition:
            ...
            return HttpResponseRedirect(
                reverse("project", args=[self.initial["campaign_pk"]])
            )
    ...
    def button_options(self, request, pk, proj_inst):
        if "next" in request.POST:
            proj_inst.save()
            next_proj = (
                Project.objects.filter(campaign=proj_inst.campaign, pk__gt=pk)
                .order_by("pk")
                .first()
                or Project.objects.filter(campaign=proj_inst.campaign).first()
            )
            return HttpResponseRedirect(reverse(self.url_path, args=[next_proj.id]))
        elif "previous" in request.POST:
            proj_inst.save()
            previous_proj = (
                Project.objects.filter(campaign=proj_inst.campaign, pk__lt=pk)
                .order_by("pk")
                .last()
                or Project.objects.filter(campaign=proj_inst.campaign).last()
            )
            return HttpResponseRedirect(reverse(self.url_path, args=[previous_proj.id]))
        elif "cancel" in request.POST:
            return HttpResponseRedirect(
                reverse("project", args=[proj_inst.campaign.pk])
            )
    ...
    def client_mailing(self, proj_inst):
        mailing, _ = Mailing.objects.get_or_create(
            project=proj_inst, notification_status=proj_inst.status, sent=False
        )
...
class ReportView(SomsoletProjectView):
    ...
    status_condition = ("technical visit", "report review", "report")
        ...
        if form.is_valid():
            if request.FILES:
                ...
                status, date_report, warn = form.report(
                    date_upload_report=date_report, is_invalid_report=report_invalid
                )
                proj_inst.status = status
                proj_inst.warning = warn
        ...
        if form.is_valid():
            if request.FILES:
                ...
                status, date_offer, warn = form.offer(
                    date_upload_offer=date_offer, is_invalid_offer=offer_invalid
                )
                proj_inst.status = status
                proj_inst.warning = warn
...
class SignatureView(SomsoletProjectView):
    ...
    def post(self, request, pk):
        ...
        return render(request, self.template_name, {"signatureform": form})
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            if request.FILES:
                ...
                status, warn = form.construction_permit(date_permit=date_permit)
                proj_inst.status = status
                proj_inst.warning = warn
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            if request.FILES:
                ...
                proj_inst.upload_delivery_certificate.name = (
                    proj_inst.name + "_" + proj_inst.upload_delivery_certificate.name
                )
                proj_inst.date_delivery_certificate = date_delivery_certificate
            return self.button_options(request, pk, proj_inst)

        return render(request, self.template_name, {"deliverycertificateform": form})
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            if request.FILES:
                ...
                proj_inst.warning = "No Warn"
                proj_inst.upload_legal_docs = form.cleaned_data["upload_legal_docs"]
                proj_inst.upload_legal_docs.name = (
                    proj_inst.name + "_" + proj_inst.upload_legal_docs.name
                )
                proj_inst.date_legal_docs = date_legal_docs
            return self.button_options(request, pk, proj_inst)

        return render(request, self.template_name, {"legalizationform": form})
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            if request.FILES:
                ...
                proj_inst.upload_legal_registration_docs.name = (
                    proj_inst.name + "_" + proj_inst.upload_legal_registration_docs.name
                )
                proj_inst.date_legal_registration_docs = date_legal_registration_docs
            return self.button_options(request, pk, proj_inst)

        return render(request, self.template_name, {"legalregistrationform": form})
...
class ProjectView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        ctx = {"project": projects_table, "filter": project_filter}
        RequestConfig(request, paginate={"per_page": 20}).configure(projects_table)
...
class DownloadCch(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):  # Autentication required
        project = Project.objects.get(
            pk=pk, campaign__engineerings=request.user.engineering
        )
        ...
        if cursor.count() == 0:
            return HttpResponseRedirect(reverse("project", args=[project.campaign.pk]))
        else:
            ...
            for measure in cursor:
                writer.writerow(
                    [project.name, measure["datetime"], measure["ai"], "Wh"]
                )
            cursor.close()
            client.close()
            return response
