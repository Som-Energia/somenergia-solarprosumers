...
class ProjectQuerySet(models.QuerySet):
    def get_project(self, project_id, user):
        try:
            ...
            return self.get(id=project_id, engineering__user=user)
...
class Project(models.Model):
    ...
    date_first_invoice = models.DateField(
        null=True, blank=True, verbose_name=_("Date First Invoice")
    )

    is_paid_first_invoice = models.BooleanField(
        default=False, verbose_name=_("Paid First Invoice?")
    )
    ...
    date_last_invoice = models.DateField(
        null=True, blank=True, verbose_name=_("Date Last Invoice")
    )

    is_paid_last_invoice = models.BooleanField(
        default=False, verbose_name=_("Paid Last Invoice?")
    )
    ...
    is_date_set = models.BooleanField(
        ...
        help_text=_(
            "Check that indicates if there is a date set to " "start installation works"
        ),
    )
    ...
    def create_offer_stage(self, date, offer_file):
        self.offer = OfferStage.objects.create(date=date, upload=offer_file)
        self.save()

    @transaction.atomic
    def create_accepted_offer_stage(self, date, check):
        self.offer = OfferStage.objects.create(date=date, check=check)
        self.save()
...
class Technical_details(models.Model):
    ...
    administrative_division = models.CharField(
        max_length=50, blank=True, verbose_name=_("Administrative division")
    )
    ...
    anual_consumption = models.CharField(
        blank=True, max_length=100, verbose_name=_("Anual Consumption (kWh)")
    )

    count_panels = models.IntegerField(
        null=True, blank=True, verbose_name=_("Solar Panels")
    )

    installation_power = models.FloatField(
        null=True, blank=True, verbose_name=_("Installation Power (kW)")
    )

    installation_model = models.CharField(blank=True, max_length=500)

    installation_singlephase_model = models.CharField(
        blank=True, max_length=500, verbose_name=_("installation singlephase model")
    )

    installation_threephase_model = models.CharField(
        blank=True, max_length=500, verbose_name=_("installation threephase model")
    )

    shadow_optimizer = models.BooleanField(
        default=False, verbose_name=_("shadow optimizer")
    )

    count_shadow_optimizer = models.IntegerField(
        null=True, blank=True, verbose_name=_("count shadow optimizer")
    )
    ...
    bateries_power = models.FloatField(
        null=True, blank=True, verbose_name=_("Batery Power (kW)")
    )

    bateries_capacity = models.FloatField(
        null=True, blank=True, verbose_name=_("Batery Capacity (kWh)")
    )
    ...
    shadow_optimizer_price = models.FloatField(
        null=True, blank=True, verbose_name=_("Shadow Optimizer (€)")
    )

    peak_power_panels_wp = models.FloatField(
        null=True, blank=True, verbose_name=_("Panel Peak Power (Wp)")
    )
    ...
    nominal_inversor_power = models.FloatField(
        null=True, blank=True, verbose_name=_("Nominal Inversor Power (Wn)")
    )
    ...
    charger_manager_price = models.FloatField(
        null=True, blank=True, verbose_name=_("Charger manager (€)")
    )
    ...
    electric_car_charger_power = models.FloatField(
        null=True, blank=True, verbose_name=_("Electric Car Charger Power (kW)")
    )

    electric_car_charger_price = models.FloatField(
        null=True, blank=True, verbose_name=_("Electric Car Charger (€)")
    )
