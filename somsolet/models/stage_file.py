...
class PrereportStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"PREINFORME [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
                str(os.path.join(base.MEDIA_ROOT, str(noti.project.prereport.upload)))
            ],
...
class OfferStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"INFORME TÈCNIC [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
...
class OfferAcceptedStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"OFERTA ACCEPTADA [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
...
class SignatureStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"CONTRACTE CLAU EN MÀ [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
...
class PermitStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"TRAMITACIÓ LLICÈNCIA D’OBRES [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
                str(os.path.join(base.MEDIA_ROOT, str(noti.project.permit.upload)))
            ],
            "from_email": base.DEFAULT_FROM_EMAIL[0],
        }
...
class SecondInvoiceStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        ...
        return {
            "subject": _(
                f"CONFIRMACIÓ DE PAGAMENT [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
                str(
                    os.path.join(
                        base.MEDIA_ROOT, str(noti.project.second_invoice.upload)
                    )
                )
            ],
            "from_email": base.DEFAULT_FROM_EMAIL[0],
...
class LegalRegistrationStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        message_params = {
            ...
            "engineering": [data["engineerings__name"] for data in campaign_data][0],
            "email": [data["engineerings__email"] for data in campaign_data][0],
            "selfconsumption_modality": _(
                "https://ca.support.somenergia.coop/article/801-que-he-de-fer-per-passar-el-meu-contracte-a-la-modalitat-amb-autoproduccio"
            ),
        }

        return {
            "subject": _(
                f"CERTIFICAT TRAMITACIÓ REGISTRE [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
                str(
                    os.path.join(
                        base.MEDIA_ROOT, str(noti.project.legal_registration.upload)
                    )
                )
            ],
            "from_email": base.DEFAULT_FROM_EMAIL[0],
...
class LegalizationStage(BaseFile):
    ...
    def email_data(self, noti, campaign_data):
        message_params = {
            ...
            "engineering": [data["engineerings__name"] for data in campaign_data][0],
            "email": [data["engineerings__email"] for data in campaign_data][0],
            "distributor_company": _(
                "https://ca.support.somenergia.coop/article/655-les-distribuidores-d-electricitat"
            ),
        }

        return {
            "subject": _(
                f"CERTIFICAT LEGALITZACIÓ [{noti.project}] - {noti.project.campaign}, compra col·lectiva de Som Energia"
            ),
            "template": self.template,
            "message_params": message_params,
            "attachment": [
                ...
                str(
                    os.path.join(base.MEDIA_ROOT, str(noti.project.ritsic_file.upload))
                ),
